...
def sieving(sqrt_N, segment_size, limit):
	...
	is_prime = [1] * (sqrt_N + 1) 
	#This needs to be a vector of size sqrt_N + 1, with elements 1.
	...
	n = 3
	print'Sieving ..'
	while low <= limit: #For N=100000, limit=100000
		#Populate sieve 
		#print('---------------------')
		...
		while s * s < high:
			if is_prime[s]: #if is_prime[3], is_prime[5], is_prime[7]... is_prime[17]
				primes.append(s)
				next.append(s * s - low)
			s = s + 2
