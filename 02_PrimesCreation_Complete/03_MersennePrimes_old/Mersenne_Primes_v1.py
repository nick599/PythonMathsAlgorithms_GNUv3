...
def main():
	...
	#result = Mersenne_prime_canditates_Creation(number, primes)
	#return Mersenne_prime_candidates
	for prime in primes:
		prime_index = primes.index(prime) + 1
		if prime_index % 10000 == 0:
		...
		if primes.index(prime)+1 <= N:
			...
			if candidate - int(candidate) == 0:
				...
				Mersenne_prime_candidates.append(int(candidate))
