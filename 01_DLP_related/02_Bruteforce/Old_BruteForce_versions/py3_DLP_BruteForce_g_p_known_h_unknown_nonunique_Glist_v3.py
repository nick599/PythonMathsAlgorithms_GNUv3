...
def main():
	...
	for num in range(0,p):
		...
		for h in range(0,p):
			...
			if temp_h == h:
				...
				h_x_list_temp.append(temp_h)
				h_x_list_temp.append(temp_x)
				h_x_list.append(h_x_list_temp)
		#a = pow(g,num,p)
		#print("a is:",a)
	...
	len_h_list = len(h_list)
	len_x_list = len(x_list)
	len_h_x_list = len(h_x_list)

	#print("len_h_list:",len_h_list)
	#print("len_x_list:",len_x_list)
	#print("len_h_x_list:",len_h_x_list)

	if not h_x_list:
		print("NO SOLUTIONS!")
	if len_h_x_list <= 20:
		print("x_soln_list:",x_list)
		print("h_soln_list:",h_list)
		print("h_x_soln_list:",h_x_list)
	else:
		x_list_5 = []
		h_list_5 = []
		h_x_list_5 = []
		...
		for x_temp in x_list:
			if x_list.index(x_temp) >= 5:
				break
			else:
				x_list_5.append(x_temp) 

		for x_temp in x_list_5:
			x_concat = str(x_concat)+" "+str(x_temp) 

		#
		for h_temp in h_list:
			if h_list.index(h_temp) >= 5:
				break
			else:
				h_list_5.append(h_temp) 

		for h_temp in h_list_5:
			h_concat = str(h_concat)+" "+str(h_temp) 

		#
		for h_x_temp in h_x_list:
			if h_x_list.index(h_x_temp) >= 5:
				break
			else:
				h_x_list_5.append(h_x_temp) 

		for h_x_temp in h_x_list_5:
			h_x_concat = str(h_x_concat)+" "+ str(h_x_temp) 

		print("1st 5 in x_list:",x_concat)
		print("1st 5 in h_list:",h_concat)	
		print("1st 5 in h_x_list:",h_x_concat)	
	...
	#consider all possible h and x values	
	#for h,x in G_list, x_list:
	#	if pow(g,x,p)==h:
	#		x_soln_list.append(x)	
	#		h_soln_list.append(h)
	...
	#		if (x,M) not in (x_values,M_values):
	#			x_values.append(x)	
	#			M_values.append(M)
