...
def main():
	...
	print('What is the power (h is raised to)?')
	h_power_initial = input()
	if h_power_initial.isdigit() is False:
		print('You have not entered a positive integer for the power. power is: ',h_power_initial,'. Please reenter.')
		sys.exit()

	#now convert p into a long:
	h_power = int(h_power_initial)

	print("---------------------")
	#print("g: ",g,", h: ",h,", p: ",p)
	print("g:",g,", p:",p,", h_power:",h_power)
	...
	#congruence is:  g**x congruent h^(h_power) mod p, where g, p and power are known, p prime.

	print("======================================")

	#Run checks on g, h & p	
	#result_ghp = ghp_checks(g, h, p) 
	result_ghp = ghp_checks(g, p, h_power)
	...
	x_values = []	
	M_values = []
	for candidate in G_list:

	#candidate = pow(h,h_power)

		#input("h ^ h_power is:",candidate)
		print("h ^ h_power is:",candidate)
		input("Waiting for user..")	
		result_dlp = dlp(g,p,h,primefile,count_nosolns,count_needlargerprimelist,answers_to_be_checked,count_brute_force_soln,h_power,candidate)		
		...
		result=check_answer(g,h,p,x, h_power)
		if result==False:
			#status=False
			print("CHECK FAILED - g: ",g,", h: ",h,", p: ",p,", x: ",x,", M: ",M,"h_power:",h_power)
			sys.exit()
		#else:
			#print("======================================")
...
def check_answer(g, h, p, x, h_power):
	#def check_answer(g, h, p, x, answers_to_be_checked):
	status=True	
	if pow(g,x,p) != pow(h,h_power):
		#print "CHECK ANSWER!!!"
...
def dlp(g, p, h, primefile, count_nosolns, count_needlargerprimelist, answers_to_be_checked, count_brute_force_soln, candidate, h_power):
...
#result = dlp(g,p,h, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked)
	...
	result = exponent_g_n_power(g,h,p, h_power)
	#return x, status
	...
	if status == True:
		...
		#exponent not found
		input("Brute force alg failed to find x..")
		sys.exit()

	#print "x is: "+str(x)
	if pow(g, x[0], p) != pow(h, h_power):
		print("CHECK ANSWER!!! - g:",g,"h:",h,"p:",p,"x:",x[0],"h_power:",h_power,"pow(g, x[0], p):",pow(g, x[0], p),"pow(h, h_power):",pow(h, h_power))
...
def exponent_g_n_power(generator,h_value, p, h_power):
	...
	while n < p:
		#print "n is:"+str(n)
		if pow(generator,n,p) == pow(h_value,h_power):
			x = n
			status=True
			break
		n = n + 1
