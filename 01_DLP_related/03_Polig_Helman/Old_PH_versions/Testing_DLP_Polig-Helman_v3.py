...
def main():
	...
	g_values = xrange(2,3)
	#g_values = xrange(2,20)
	p_values = primes[0:1001]
	h_values = xrange(2,201)
	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g == p:
				...
				#raw_input("What do we want to calculate instead of order??")
				#Calculate order							
				#order_result = calc_order(g, p, count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime)
				...
				for h in h_values:								
					...
					if h >= p:
						...
						#print "type(x_final): "+str(type(x_final)) #str
						#print "type(int(x_final)): "+str(type(int(x_final))) #
						...
						if h not in G_list:
							#print "h is not in Group G. Hence no solutions!"
...
def dlp(g, h, p, primefile, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked):	
	...
	if result is False:
		...
		result_polig = polig_helman(primes_list, powers, g, h, p, count_nosolns, count_gi_hi_equal_1, count_diffeq0, count_x_equals_0, count_gi_equal_1_hi_ntequal_0_1, count_normal_soln, count_gi_equals_hi, count_gi_hi_equal_1)
	
		#return x, count_nosolns, count_gi_hi_equal_1, count_gi_equal_1_hi_ntequal_0_1, count_x_equals_0, count_gi_equals_hi, count_normal_soln, x_moduli_final, count_diffeq0		
	...
	return x_final, count_nosolns, count_gi_hi_equal_1, count_gi_equal_1_hi_ntequal_0_1, count_x_equals_0, count_gi_equals_hi, count_normal_soln, x_moduli_final, count_diffeq0

def polig_helman(primes_list, powers, g, h, p, count_nosolns, count_gi_zi_equal_1, count_diffeq0, count_x_equals_0, count_gi_equal_1_hi_ntequal_0_1, count_normal_soln, count_gi_equals_hi, count_gi_hi_equal_1):
	...
	for C in C_list:
		...
		if gi[i]==1 and hi[i] == 1:
			g_init = pow(g, Wi[i])
			h_init = pow(h, Wi[i])
			#print "g_init is: "+str(g_init)
			#print "h_init is: "+str(h_init)

			x_status = False
			for number in xrange(1, 2*p):
				a = pow(g_init,number)
				#print "pow(g_init,number) is: "+str(a)				
				if a == h_init:
					x_init = number
					x_status = True
					break
			...
			if x_status == True:
				#print "x is "+str(number)
				x.append(x_init)
				print str(x_init)+" appended to x"	
				moduli = pow(qi[i],ei[i])
				x_moduli.append(moduli)
				print "Moduli: "+str(moduli)
				#raw_input("Waiting for user..")
			else:
				print "x not found! g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", g_init: "+str(g_init)+", h_init: "+str(h_init)
	...
	return x, count_nosolns, count_gi_hi_equal_1, count_gi_equal_1_hi_ntequal_0_1, count_x_equals_0, count_gi_equals_hi, count_normal_soln, x_moduli, count_diffeq0
