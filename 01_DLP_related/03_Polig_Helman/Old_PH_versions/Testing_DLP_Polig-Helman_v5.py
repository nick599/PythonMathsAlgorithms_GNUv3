	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g == p:
				...
				count_nosolns = count_nosolns + len(xrange(1,p))	
			else:				
...
def dlp(g, p, h, primefile, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked):	
	...
	if result is False:
		...
		if x_status == False:
			...
			#raw_input("Waiting for user..")
			return 0, count_nosolns, count_gi_hi_equal_1, count_gi_equal_1_hi_ntequal_0_1, count_x_equals_0, count_gi_equals_hi, count_normal_soln, 0, count_diffeq0, x_status
		elif x_status == True:
			#print('========================')
			print "x's are: "+str(x)
			print "x_moduli are: "+str(x_moduli)
