...
def main():
	...
	sum_g_prime_values = 0
	for g in g_values:
		if isprime(g) == True:
			sum_g_prime_values = sum_g_prime_values + g

	print("sum_g_prime_values:",sum_g_prime_values)
	print("len_g_values:",len(g_values))
	...
	#print("len(p_values):",len(p_values))	
			...
			for p in p_values:
				sum_prime_values = sum_prime_values + p
			total_combinations = (sum_prime_values - len(p_values))*len(g_values) - sum_g_prime_values
			print("Test: 1, p_values: 0:101, len_p_values:",len(p_values),", total_combinations <=",total_combinations)
			txt_to_append = "*** Test: 1, p_values: 0:101, len_p_values: "+str(len(p_values))+", total_combinations <= "+str(total_combinations)+" ***\n"
			...
			for p in p_values:
				sum_prime_values = sum_prime_values + p
			total_combinations = (sum_prime_values - len(p_values))*len(g_values) - sum_g_prime_values
			print("Test:",num,", p_values:",((num-1)*100)+1,"to",(num*100)+1,", len_p_values:",len(p_values),", total_combinations <=",total_combinations)
			txt_to_append = "*** Test: "+str(num)+", p_values: "+str(((num-1)*100)+1)+" to "+str((num*100)+1)+", len_p_values: "+str(len(p_values))+", total_combinations <= "+str(total_combinations)+" ***\n"
			txtfile_append(txt_to_append,output_path)
			#print("---------------------------")
		
		test(g_values, p_values, primes, sample_range, output_path)			
...
def test(g_values, p_values, primes, sample_range, output_path):
	...
	#txt_to_append = "p_values: "+str(p_values)+", Grand_total_values: "+str(Grand_total_values)+"\n"
	#txtfile_append(txt_to_append,output_path)
			...
			for num in sample_range:
				...
				if g != p:				
					...
					if result is False:
						...
						h_values_individ_dep_on_p = []
						h_values_individ_dep_on_p_total = 0
						...
						for h in G_list:
							if g == h:					
								...
								h_values_individ_dep_on_p.append(c_after_result_polig)
								h_values_individ_dep_on_p_total = h_values_individ_dep_on_p_total + c_after_result_polig
									
								#print("h: ",h,", all hi:",hi)

								if h == G_list[-1]:							
									#print("p:",p,", len_G_list:",len(G_list),", h_power_values:",h_power_values_to_append,", gi_values_to_append:",gi_values_to_append)
									#store max, min, and range of h_values_individ_dep_on_p 
									max_h_values_individ_all = max(h_values_individ_dep_on_p)
									#print("max_h_values_individ_all:",max_h_values_individ_all)
									#min_h_values_individ_all = min(h_values_individ_dep_on_p)
									#print("min_h_values_individ_all:",min_h_values_individ_all)
									#range_h_values_individ_all = max_h_values_individ_all - min_h_values_individ_all
									#print("range_h_values_individ_all:",round(range_h_values_individ_all,8))
								
									mean_h_values_individ_all = float(h_values_individ_dep_on_p_total)/len(h_values_individ_dep_on_p)

									#position_max_h = h_values_individ_dep_on_p.index(max_h_values_individ_all)
									#print("position_max_h:",position_max_h)								
									h_value_max = G_list[h_values_individ_dep_on_p.index(max_h_values_individ_all)]
									#h_value_max = G_list[position_max_h]
						...
						if mean_h_values_individ_all >= 0.003:
							#print("p:",p,", len_G_list:",len(G_list),", h_power_values:",h_power_values_to_append,", gi_values_to_append:",gi_values_to_append)
							#print("h_value_max:",h_value_max,", max_h_values_individ_all:",round(max_h_values_individ_all,8))
							txt_to_append = "g:"+str(g)+", p:"+str(p)+", len_G_list:"+str(len_G_list)+", max_h_values:" +str(round(max_h_values_individ_all,6))+", h_value_max:"+str(h_value_max)+", mean_h_values:"+str(round(mean_h_values_individ_all,6))+", h_power_values:"+str(h_power_values_to_append)+", gi_values:"+str(gi_values_to_append)+", prime_factors:"+str(prime_factors)+", powers:"+str(powers)+"\n"
