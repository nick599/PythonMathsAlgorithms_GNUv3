	...
	for g in g_values:
		...
		for p in p_values:
			...
			for num in sample_range:
				...
				if g != p:				
					...
					for prime in prime_factors:
						#i = prime_factors.index(prime)						
						#gi_values.append(pow(g,int(p_minus_1 / pow(prime_factors[i],powers[i])),p))
						gi_values.append(pow(g,int(p_minus_1 / pow(prime,powers[prime_factors.index(prime)])),p))
						...
						for h in range(1, p):								
							...
							if h in G_list:
								...
								if g == h:					
									...
									result_polig = polig_helman(prime_factors, powers, g, p, h, p_minus_1, gi_values)
			...
			if c_after_result_polig_count == 0:
				...
				mean_polig_time = str(round(float(c_after_result_polig_total)/c_after_result_polig_count,5))
			...
			if c_after_h_values_count == 0:
				mean_h_values_time = "(none)"
			else:
				mean_h_values_time = str(round(float(c_after_h_values_total)/c_after_h_values_count,5))
			...
			txt_to_append = "concat_g_p: "+concat_g_p+", mean_time: "+mean_time+", max_time: "+max_time+", min_time:"+min_time+", mean_h_values_time: "+mean_h_values_time+", mean_g_equals_h_time: "+mean_g_equals_h_time+", mean_h_in_G_list_time: "+mean_h_in_G_list_time+"\n"
...
def polig_helman(primes_list, powers, g, p, h, p_minus_1, gi):
