...
def main():
	...
	for num in range(1,11):	
		...
		result = test(g_values, p_values, primes, sample_range)
...
def test(g_values, p_values, primes, sample_range):
	#prime_list_path="/home/mint/Desktop/"
	#prime_list_filename="primes_upto_100000.csv"
	#primefile=prime_list_path + prime_list_filename
	#print("primefile currently is: ",primefile)
	...
	for g in g_values:
		...
		for p in p_values:
			...
			for num in sample_range:
				#if num % 5 == 1:			
				#	print("Sample test number:",num,"of",len(sample_range))			
			...
			mean_time = round(float(c_after_g_nt_p_test_total)/len(time_values),6)
			g_p_time_values.append(mean_time)
			max_time = round(max(time_values),6)
			g_p_time_values.append(max_time)
			min_time = round(min(time_values),6)
...
def dlp(g, p, h, primes):	
	...
	if result is False:
		...
		result_polig = polig_helman(primes_list, powers, g, p, h)
...
def polig_helman(primes_list, powers, g, p, h):
