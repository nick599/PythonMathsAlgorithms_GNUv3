...
print("Version:",version,". 24/05/2018.")
...
def main():
	...
	output_files=["output_v"+str(version)+"_"+str(sample_range[0])+".txt","output_v"+str(version)+"_"+str(sample_range[1])+".txt","output_v"+str(version)+"_"+str(sample_range[2])+".txt"]
	...
	for number in sample_range:
		...
		for num in range(1, number + 1):
			...
			if len(factors)==2:
				...
				if isprime(factors[0]) == True and isprime(factors[1]) == True:
					...
					if not p_q_list_for_append:					
						p_q_list_for_append = str(concat_p_q)
					else:
						p_q_list_for_append = p_q_list_for_append + "," + str(concat_p_q)
		...
		print("number:",number,", len p_q_list:",len(p_q_list),", c_after_loop:",c_after_loop)
		data="number:"+str(number)+", len p_q_list:"+str(len(p_q_list))+", c_after_loop:"+str(c_after_loop)+"\n"
