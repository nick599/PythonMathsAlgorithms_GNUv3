	...
	while error==False:	
		for g in g_values:
			for p in p_values:
				...
				concat = str(g)+"_"+str(p)				
				if g == p:
					count_nosolns = count_nosolns + len(h_values)
				elif h_values_no_solution == True:
					print "h >= p. h_values_no_solution = True. Breaking.."
					break				
				elif concat in not_prim_root_list:
					print str(g)+" is not a primitive root mod "+str(p)
					break
				elif concat in order_false_list:
					print "order for "+str(concat)+" is not in the form q**e."
					break
				else:				
					#raw_input("Waiting for user..")				
					for h in h_values:								
						if h >= p:
							...
							if exponent_g_n_status == True:
								...
								#print('Running dlp()..')
								result = dlp(g,h,p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime, count_primroot, prim_root_list, not_prim_root_list, order_false_list)
...
def dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_a_notprime, count_primroot, prim_root_list, not_prim_root_list, order_false_list):

	print "-------------------------------------------------"
	print "Running dlp("+str(g)+", "+str(h)+", "+str(p)+"..)"
	...
	if status_isprime_a == False:					#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			if concat in prim_root_list:
				print "Previously calculated - "+str(g)+" is a prim root mod "+str(p)	
				count_primroot = count_primroot + 1
				
				#Now check if concat is order_false_list
				if concat in order_false_list:
					print "Previously calculated - "+str(concat)+" is NOT in form q**e !"
					x_final = "No solns!"			
					...
					print "Need to calculate & check order for: "+str(concat)
				...
				if prim_root_status is False:
					...
					not_prim_root_list.append(concat)
					x_final = "No solns!"
					return x_final, count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list
					#sys.exit()								#O(1)
			
				else:
					print(str(g)+' is a primitive root mod '+str(p))	
					prim_root_list.append(concat)
					count_primroot = count_primroot + 1			#O(1)

					if concat in order_false_list:
						print "Order for "+str(concat)+" previously calculated - is NOT of form q**e !"	
						...
						if order == 1:						#O(1)
							...
							order_false_list.append(concat)
							...
							if order_status==False:						#O(1)
								...
								x_final = "No solns!"
								return x_final, count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list, order_false_list
								#sys.exit()							
							else:
								...
								if calc_xi_status == True:
									...
									return x[0], count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, x_moduli[0], answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list, order_false_list
									...
									return 0, count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list, order_false_list
