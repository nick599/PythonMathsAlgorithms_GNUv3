...
def prim_root(g,p,primes):
	...
	#Assuming p is prime: 		Best case: O()		Worst case: O(n**4 + n**3 + 8n**2 + 16n + sqrt(n) + 31)
	...
	result=factorise(ep, primes)				#Worst: O(3n**2 + 12n + 21)	#Best: O(27 ???) - assuming there are elements in "list"
	...
	for prime in prime_factors:				#O(n)	#Worst: O(n**4 + n**3 + 5n**2 + 4n)	#Best: O(9)
		if status==True:				#O(1)	#Worst: O(n**3 + n**2 + 5n + 4)		#Best: O(8)
			#print "================="
			#print "prime is: "+str(prime)
			power=1					#O(1)
			power_index = 0				#O(1)
			#print "power_index is: "+str(power_index)
			while power < powers[power_index]:	#O(n)	#Worst: O(n**3 + n**2 + 5n) #Best: O(3)
				#print "------------------"
	...
	if result == 0:							#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			if result is False:
				...
				result = exponent_g_n(g, 1, p)				#Worst: O(n**2 + 7n + 6)	#Best: O(8)
				#return x, status
				order = result[0]
				status = result[1]

				#print "order is: "+str(order)
				sqrt_order=math.floor(math.sqrt(order))			#O(2)
				...
				if order == 1:
					...
					result = calc_q_e(primes, order, p)		#Worst: O(2n**2+7n+4)	#Best: O(12)
					#return q, e, result
...
def calc_q_e(primes, k, p):
	...
	n=1				#O(1)
	#print "p is: "+str(p)
	for prime in primes:		#O(n)	#Worst: O(2n**2+7n)	#Best: O(8)
		#print "=================="
		n = 1					
...
def exponent_g_n(generator,h_value, p):
	...
	#Worst: O(n**2 + 7n + 6)	#Best: O(8)

	n=1						#O(1)
	x=0						#O(1)
	status=False					#O(1)
	while n < p:					#O(n)	#Worst: O(n**2 + 7n)
		#print "n is:"+str(n)
...
def factorise(N,list):		
	...
	#Worst: O(3n**2+12n+21)	#Best: O(27 ???) - assuming there are elements in "list"
	...
	result2=calc_primes_powers_remainder(list,N_remainder)		#Worst: O(3n**2+12n+7)	#Best: O(13 ???) - assuming there are elements in "list" 
...
def calc_primes_powers_remainder(list, N_remainder):
	...
	#Worst: O(3n**2+12n+7)	#Best: O(13 ???) - Assuming elements in "list"
	...
	else:					#O(1)		#Worst: O(3n**2+12n+1)	#Best: O(6)
		#There are elements in primes list
		# N_remainder is not in isliced_primes:
		for prime in list:					#O(n)	#Worst: O(3n**2+12n) #Best: O(5)
			...
			if N_remainder <> 1:					#O(1)	#Subtotal: O(3n+11)
