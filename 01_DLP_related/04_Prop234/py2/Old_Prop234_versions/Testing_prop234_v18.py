				...
				if g == p:
					...
					for h in h_values:								
						...
						if h >= p:
							...
							#h_position = list(h_values).index(h)
							...
							#raw_input("Waiting for user..")
							break													
						elif str(g)+"_"+str(h) in order_false_list:
							count_nosolns = count_nosolns + 1
						
						#elif concat_g_h in order_false_list:
							#print "order for "+str(concat_g_h)+" is not in the form q**e."
							#order_no_solution = True
							...
							#Calculate order							
							order_result = calc_order(g, p, count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, order_false_list, primes)

							#return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, order_false_list, q, e, order_status
							...
							#print('Running dlp()..')
								...
								#result = dlp(g,p,h, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime, count_primroot, prim_root_list, not_prim_root_list, order_false_list)
			
								#return x[0], count_nosolns, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, order_false_list

								#return (0 or x[0]), count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, (0 or x_moduli[0]), answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list, not_prim_root_list, order_false_list

								print "result[0] is: "+str(result[0])
								print "type(result[0]) is: "+str(type(result[0]))
								x_final = result[0]
								...
								count_x_equals_0 = count_order_not_prime + result[7]
								count_needlargerprimelist = count_needlargerprimelist + result[8]
								count_q_e_found = count_q_e_found + result[9]
								count_q_e_not_found = count_q_e_not_found + result[10]
								count_notprime = count_notprime + result[11]
								#prim_root_list = result[13]
								#not_prim_root_list = result[14]
								order_false_list = result[12]
...
def dlp(g, p, h, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime, order_false_list):
	...
	if status_isprime_a == False:					#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			#Now check if concat_g_h is order_false_list
			#concat_g_h = str(g)+"_"+str(h)

			if str(g)+"_"+str(h) in order_false_list:
				print "Previously calculated - "+str(concat_g_h)+" is NOT in form q**e !"
				#print "Previously calculated - "+str(g)+"_"+str(h)+" is NOT in form q**e !"
				x_final = "No solns!"			
				return x_final, count_nosolns, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, order_false_list
			else:		
				if order_status==False:						#O(1)
					...
					if calc_xi_status == True:
						...
						return x[0], count_nosolns, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, x_moduli[0], answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, order_false_list
						...
						#return 0, count_nosolns, count_primroot, count_order_not_prime, count_calc_xi_no_solns, count_normal_soln, 0, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_q_e_found, count_q_e_not_found, count_a_notprime, prim_root_list, not_prim_root_list		
...
def calc_order(g, p, count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, order_false_list, primes):
	...
	#print "Need to calculate & check order for: "+str(g)+"_"+str(h)
	#print "Need to calculate & check order for: "+str(concat_g_h)					
