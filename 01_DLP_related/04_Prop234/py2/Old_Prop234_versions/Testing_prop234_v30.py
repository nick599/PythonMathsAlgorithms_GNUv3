...
#Version 30. 17/04/2018.
#Programmed & tested in Python 2.76 only
#This program tests the algorithm in prop234.py - solving multiple Discrete Log Problems (DLPs) specified by user in ranges, via Proposition 2.34 (in J Hoffstein, J Pipher & J Silverman) using order of g mod p, where p is a prime number.
...
def main():
	...
	print "Grand_total_values: "+str(Total_values)
	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g == p:
				count_nosolns = count_nosolns + len(xrange(1,p+1))
	...
	if status_isprime_a == False:					#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			if calc_xi_status == True and xi_found == True:
				...
				#raw_input("calc_xi_status = False ! ")
...
def calc_q_e(primes, order, p):
	...
	#***********raw_input("What should be used for calculating q & e - pow(prime,n) or pow(prime,n,p)??")***********
