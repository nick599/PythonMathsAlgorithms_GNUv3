...
def main():
	...
	#result = dlp(g,h,p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_notprime)

	#result = dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_calc_xi_no_solns, count_notprime):

	result = dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime)
...
def prim_root(g,p,primes):
	...
	#Assuming p is prime: 		Best case: O(46)	Worst case: O(4n**2 + 12n + sqrt(n) + 37)
	#Assuming p is not prime: 	O()

	#print('Checking if '+str(g)+' is a primitive root mod '+str(p)+' ..')
	status=True						#O(1)	
	result = euler_phi(p, primes)				#Worst: O(sqrt(n)+9) when n is prime	#best:	O(3) when n=1
	#return a, status

	ep=result[0]						#O(1)
	#print "euler_phi("+str(p)+") is: "+str(ep)		

	result=factorise(ep, primes)				#Worst: O(n**2 + 8n + 21)	#Best: O(27 ???) - assuming there are elements in "list"
	...
	print "Now testing if "+str(g)+" is a primitive root mod "+str(p)	#O(1)
	#testing if g is a primitive root mod p
	for prime in prime_factors:				#O(n)	#Worst: O(3n**2 + 4n)	#Best: O(9)
		if status==True:				#O(1)	#Worst: O(3n + 4)	#Best: O(8)
			...
			while power < powers[power_index]:	#O(n)	#Worst: O(3n + 1) #Best: O(3)
				...
				#if g**(ep / powers[power_index]**power) % p == 1:	#O(n**2 + n + 2) 	#Worst: O(n**2 + n + 4) #Best: O(3)
					status=False					#O(1)
...
def euler_phi(n, primes):				
	...
	#Worst: O(sqrt(n)+9) when n is prime 
	#best:	O(3) when n=1
...
def dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime):
	...
	if result == 0:							#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			if result is False:
				print(str(g)+' is not a primitive root mod '+str(p)+'! Exiting ...') 	#O(1)		
				sys.exit()								#O(1)
			else:
				...
				print "primes_list is: "+str(primes_list)		#O(1)
				print "powers are: "+str(powers)			#O(1)
...
def calc_q_e(primes, k, p):
	...
	for prime in primes:		#O(n)	#Worst: O(n**2+7n)	#Best: O(8)
		...
		#while result is False and prime < p and n <= k and prime**n <= k:	#O(n)	#Worst: O(2n+7)	#Best: O(7)
			#print "-----------------"
			#print "n is: "+str(n)+", k is: "+str(k) 
	...
	for q_power in q_powers:
		...
		if calc_xi_status == True:
			if q_power==1:							#total (incl top lev if) = 
				...
				#lhs				
				#print "g**qi**(ei-1) is: "+str(g**qi**(ei-1))				
				...
				#rhs
				#print "h**qi**(ei-1) is: "+str(h**qi**(ei-1))		
				...
				if xi[xi_number-1] >= 0:
					...
					print("xi["+str(xi_number-1)+"] is negative !!! CHECK")							#O(1)
					raw_input("Waiting for user..")										#O(1)
					sys.exit()												#O(1)
...
def factorise(N,list):		
	...
	result2=calc_primes_powers_remainder(list,N_remainder)		#Worst: O(n**2+8n+7)	#Best: O(13 ???) - assuming there are elements in "list" 
