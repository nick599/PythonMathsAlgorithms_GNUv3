...
def main():
	...
	#result = dlp(g, h, p, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime)	#O(1)

	result = dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime)	#O(1)
...
#def prim_root(g,p,primes):
	...
	status = True						#O(1)	
	result = euler_phi(p)					#Worst: O(sqrt(n)+9) when n is prime	#best:	O(3) when n=1
	...
	if status_ep == False:
		...
		result2 = calc_powers(factors)				#Worst: O(factors)
...
#def dlp(g, h, p, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime):
...
def dlp(g, h, p, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_notprime):
	...
	status_ghp = result[0]
	status_not_prime = result[1]

	if status_not_prime == False:

	elif status_not_prime == False:							#O(1)
		count_notprime = count_notprime + 1				#O(1)
		sys.exit()							#O(1)
	else:
		#define prime list
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			result=prim_root(g,p)			#Worst case: O(???)
