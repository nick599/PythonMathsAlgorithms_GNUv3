...
def ghp_checks(g,h,p,floor_sqrt_p, count_notprime):	
	...
	if a == False:					#O(1)	#Best case: O(5)
		...
		q=(p-1)/2											#O(1)
		if isprime(q)==False:										#O(sqrt(n)+1)
			print str((p-1)/2)+" is not prime. Hence "+str(g)+" is not a primitive root mod "+str(p)
			status=0												#O(1)
			sys.exit()												#O(1)
	return status, a
