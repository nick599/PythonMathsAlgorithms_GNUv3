	...
	if result == 0:							#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			if result is False:
				...
				if pow(g,x_final,p) <> h:			#O(1)
				#if g**x_final % p <> h:
					#print "CHECK ANSWER!!!"
					print "CHECK FAILED - g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", x_final: "+str(x_final)+", x_moduli: "+str(x_moduli)
					answers_to_be_checked.append("g: "+str(g)+" h: "+str(h)+" p: "+str(p)+" x: "+str(x))
					sys.exit()
