...
def dlp(g, h, p, primefile, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked):
	...
	if result is False:
		...
		#use exponent function to get order q of g mod p
		result = exponent_g_n(g, 1, p)
		#return x, status
		order = result[0]
		status = result[1]

		#now order = q**e. Need to find q and e.
		
		#create list of squares upto p-1


		#test if order is in squares list



		#if in list take math.sqrt(), to obtain q, and e = 2
