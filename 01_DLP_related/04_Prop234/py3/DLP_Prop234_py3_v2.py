	...
	if g > p:
		...
		if isprime(p) == True:
			...
			if len(G_list) < 20:			
				print("G_list:",G_list)
			if h not in G_list:
				print("h:",h, "is not in G_list. Hence no solutions!")
				sys.exit()
...
def dlp(g, p, h, primefile, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_q_e_found, count_q_e_not_found, count_order_not_prime, count_calc_xi_no_solns, count_a_notprime):
	...
	#print("math.sqrt(p):",math.sqrt(p))
	#print("math.floor(math.sqrt(p)):",math.floor(math.sqrt(p)))
	...
	if status_isprime_a == False:					#O(1)
		...
		if sqrt_p > largest_prime:				#O(1)	#Subtotal: O(4)
			...
			#use order_g_p() function to get order of g mod p
			result = order_g_p(g, p)				#Worst: O(n + 7)	#Best: O(8)
			#return x, status
...
def order_g_p(g, p):
	#print "Running order_g_p("+str(g)+", "+str(p)+").."
