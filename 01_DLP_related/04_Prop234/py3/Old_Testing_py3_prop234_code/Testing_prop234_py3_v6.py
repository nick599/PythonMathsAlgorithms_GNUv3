	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g > p:			
				...
				answers_to_be_checked=result[5]
				...
				previous_calc_orders=result[12]
	...
	if g == 1:
		...
		if str(order)+"_"+str(p) in q_e_nf_order_p_list:
			...
			#concat_g_h = str(g)+"_"+str(h)
				...
				#x_moduli_final
				return x_final, count_nosolns, count_calc_xi_no_solns, count_normal_soln, x_moduli_final, answers_to_be_checked, count_x_equals_0, count_needlargerprimelist, count_notprime, q_e_nf_order_p_list, count_xi_not_found, count_calc_xi_status_false, previous_calc_orders
