	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g > p:			
				...
				result = Calc_prop_234(g_new,p,count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime, previous_calc_g_p, previous_calc_orders)

			elif str(g)+"_"+str(p) not in previous_calc_g_p:
			
				result = Calc_prop_234(g,p,count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime, previous_calc_g_p, previous_calc_orders)
...
def Calc_prop_234(g,p,count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime, previous_calc_g_p, previous_calc_orders):
						...
						#for h in h_values:								
							...
							if h >= p:
								...
								if order_status == True:
									...
									result = dlp(g_new, p, h, count_nosolns, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_calc_xi_no_solns, count_notprime, q_e_nf_order_p_list, primes, q, e, n_prime, count_xi_not_found, count_calc_xi_status_false, lhs)
