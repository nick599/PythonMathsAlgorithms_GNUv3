	...
	for g in g_values:
		...
		for p in p_values:
			...
			if g > p:			
				...
				result = Calc_prop_234(g_new,p)

			elif str(g)+"_"+str(p) not in previous_calc_g_p:
			
				result = Calc_prop_234(g,p)
...
def abc(g,p):
				...
				if str(g)+"_"+str(p) in previous_calc_g_p:
					...
					g_p_to_append=[]
					g_p_to_append.append(str(g_new)+"_"+str(p))
				...
				if g == 1:
						...
						if h >= p:
							...
							if order_status == True:
								...
								if x_final != "No solns!":						
									...
									#Check answers
									if pow(g_new, x_final, p) != h:
										print("CHECK x_final! g: "+str(g_new)+", h: "+str(h)+", p: "+str(p)+", x_final: "+str(x_final)+", x_moduli: "+str(x_moduli))
										input("Waiting for user..")

									elif pow(g_new, x_final + x_moduli_final, p) != h:
										print("CHECK x_moduli_final! g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", x_final: "+str(x_final)+", x_moduli_final: "+str(x_moduli_final))
										input("Waiting for user..")
					...
					print("Order for g:",g_new,",p:",p,"does not match criteria - not in previous_calc_g_p!")
