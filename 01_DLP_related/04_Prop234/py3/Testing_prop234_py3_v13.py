			...
			if g % p == 0:
				...
				if g_new == 1:
					...
					if g_new_p not in previous_calc_g_p:
						...
						print("count_order_not_prime before calc_prop_234:",count_order_not_prime)
						print("count_order_prime before calc_prop_234:",count_order_prime)
						...
						print("count_order_not_prime after calc_prop_234:",count_order_not_prime)
						print("count_order_prime after calc_prop_234:",count_order_prime)
				...
				print("count_order_not_prime before calc_prop_234:",count_order_not_prime)
				print("count_order_prime before calc_prop_234:",count_order_prime)
				input("g_p in previous_calc_g_p - Waiting for user..")
...
def Calc_prop_234(g,p,count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime, previous_calc_g_p, previous_calc_orders, count_x_equals_0, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_calc_xi_no_solns, count_notprime, count_xi_not_found, count_calc_xi_status_false):
	...
	print("count_order_not_prime is:",count_order_not_prime)	# 
	print("count_order_prime is:",count_order_prime)	# 
	...
	if str(g)+"_"+str(p) in previous_calc_g_p:
		...
		input("calculated previously - Waiting for user..")
		order=""
		...
		if g == 1:
			...
			if g_p_to_append in previous_calc_orders:
				...
				print("count_order_prime now:",count_order_prime)	#
				input("waiting for user..")
				...
				#input("waiting for user..")					
				print("count_order_prime before calc_order() is:",count_order_prime)	# 0!
				...
				print("g:",g,"p:",p,"count_order_not_prime after calc_order():",count_order_not_prime)
				print("g:",g,"p:",p,"count_order_prime after calc_order():",count_order_prime)
				input("After calc_order - Waiting for user..")
				    ...
				    #concat_g_h = str(g)+"_"+str(h)
						...
						if h >= p:
							...
							if order_status == True:
								...
								print("count_order_not_prime before dlp() is:",count_order_not_prime)
								print("count_order_prime before dlp() is:",count_order_prime)
								...
								print("count_order_not_prime after dlp() is:",count_order_not_prime)
								print("count_order_prime after dlp() is:",count_order_prime)
								input("After dlp() - Waiting for user..")
								...
								input("Error order status - waiting for user..")								
							    #error=True
			...
			if calc_xi_status == True and xi_found == True:
				...
				if x_initial == 0:		
					...
					if moduli == 0:
						...
						input("CHECK moduli - Waiting for user..")
					else:
...
def calc_order(g, p, count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, primes, n_prime, count_order_prime, previous_calc_g_p):
	...
	if status == False:					#O(1)
		...
		input("Order not found - Waiting for user..")
		return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, q, e, order_status, n_prime, count_order_prime, order, lhs
	elif order == 1:					#O(1)
		q = order						#O(1)
		e = 1							#O(1)
		...
		print("p:",p,"order=1, count_order_prime:",count_order_prime)
		input("Order = 1 - Waiting for user..")		
		return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, q, e, order_status, n_prime, count_order_prime, order, lhs
		...
		print("p:",p,"order:",order,"count_order_not_prime:",count_order_not_prime)
		print("p:",p,"order:",order,"count_order_prime:",count_order_prime)
		input("order in n_prime - Waiting for user..")
		return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, q, e, order_status, n_prime, count_order_prime, order, lhs
		...
		print("p:",p,"order:",order,"count_order_not_prime:",count_order_not_prime)
		print("p:",p,"order:",order,"count_order_prime:",count_order_prime)
		input("order is prime - Waiting for user..")
		return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, q, e, order_status, n_prime, count_order_prime, order, lhs
		...
		print("order:",order,", count_order_prime now:",count_order_prime)
		...
		if order_status == False:
			...
			print("order_status is: "+str(order_status))
			lhs = 0
			input("Unknown order status! - Waiting for user..")
			concat_order_p = str(order)+"_"+str(p)
			q_e_nf_order_p_list.append(concat_order_p)
		return count_nosolns, count_order_not_prime, count_q_e_found, count_q_e_not_found, q_e_nf_order_p_list, q, e, order_status, n_prime, count_order_prime, order, lhs
