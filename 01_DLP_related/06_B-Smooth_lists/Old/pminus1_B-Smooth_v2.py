	...
	for prime in primes:
		if primes.index(prime) + 1 <= N:
			...
			if B_Smooth(candidate, primes, beta, beta_index) == True:
				...
				print(candidate," added to pminus1_Beta_Smooth_candidates")
			#input("Waiting for user..")

	#SG_prime_candidates = result[0]
	
	print('len pminus1_Beta_smooth_candidates is:',len(pminus1_Beta_Smooth_candidates))
	print('First 10 pminus1_Beta_smooth_candidates are:',pminus1_Beta_Smooth_candidates[0:10])
	print('Last 10 pminus1_Beta_smooth_candidates are:',pminus1_Beta_Smooth_candidates[-10:])
