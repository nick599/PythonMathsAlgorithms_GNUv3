...
def main():
	...
	print('Running dlp()..')
	result = dlp(g,h,p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_primroot_sub)
...
def chinese_remainder(x, x_moduli):
	...
	for number in xrange(0,len(x)):
		...
		if not cong:
			...
			print "Subtracting cong_x[0] from current x[number].."
			c = x[number] - cong_x[0] 
			print "c now is: "+str(c)				#c=
			
			print "cong_moduli[0] to use in inverse is: "+str(cong_moduli[0]) 		#number=
			print "x_moduli[number] to use in inverse is: "+str(b)	#moduli=

			#now want to find (cong_moduli[0]**-1) mod(b)		
			f = calc_modinverse(cong_moduli[0], 1, b)
			...
			x_new = cong_x[0] + (cong_moduli[0] * k)
...
def euler_phi(n, primes):
	...
	if isprime(n,floor_sqrt_n) == 1:
		...
		if remainder:
			...
			sys.exit()
		c = 1
		print "len(primes) is: "+str(len(primes))
		for i in xrange(0, len(primes)):			
			print "i is: "+str(i)
			print "primes["+str(i)+"] is: "+str(primes[i])
...
def dlp(g, h, p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_primroot_sub):
	...
	for C in C_list:
		...
		if Bi[i]==0 and zi[i]==1:
			...
			print'----NEED TO CHECK MODULI!!!----'
			#NEED TO CHECK MODULI!!!
			diff = a_exp_x_eq_1(a,p)			
