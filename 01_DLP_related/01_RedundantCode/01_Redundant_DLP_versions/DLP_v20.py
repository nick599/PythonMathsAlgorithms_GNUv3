...
def prop_234(Bi, zi, qi, ei, p):
	...
	for q_power in q_powers:
		...
		if q_power==1:
			...
			#ie solving (g**q**(e-1))*x_0 = h**q**(e-1), mod q**(e-1) 
			#eg g=3, h=5, p=101. Hence (3**5**1)*x_0 = 101**5**1, mod 5**1 
			lhs= g**qi**(ei-1)%(qi**(ei-1))
			print("lhs is: "+str(lhs))
			rhs= h**qi**(ei-1)%(qi**(ei-1))
			...
			##############################################
			# Need to do euclid forward and reverse next for g**(-x_0) mod q**(e-1)
			#f = modinv(a, m)
			#f = a**(-1) mod q**(e-1)
			print("qi is: "+str(qi))
			print("ei is: "+str(ei))
			...
			f = modinv(g, moduli) #this is for when x_0 = 1 
			print(str(g)+"**(-1) mod "+str(moduli)+" is: "+str(f))
	
			#let j = a**(-2) mod q**(e-1)
			#aj = a**(-1) mod q**(e-1) = f mod q**(e-1)
			#j = f*a**(-1) mod q**(e-1) = f*f mod q**(e-1)
			#Hence j = modinv(f**2,q**(e-1))
			j = modinv(f**2, moduli) #this is for when x_0 = 2
			print("j is: "+str(j))
			print(str(g)+"**(-2) mod "+str(moduli)+" is: "+str(j))

			#let k = a**(-3) mod q**(e-1) #this is for when x_0 = 3
			#ak = a**(-2) mod q**(e-1)= j mod q**(e-1)
			#k = j*a**(-1) mod q**(e-1) = f*f*f mod q**(e-1)
			k = modinv(f**3, moduli)
			print("k is: "+str(k))
			print(str(g)+"**(-3) mod "+str(moduli)+" is: "+str(k))
