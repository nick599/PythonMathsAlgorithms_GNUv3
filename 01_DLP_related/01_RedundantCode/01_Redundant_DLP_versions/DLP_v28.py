...
def prop_234(Bi, zi, qi, ei, p):
	...
	#for j in xrange(0, e+1): 
		...
		if q_power==1: 
			print("Initialising x as: xi["+str(q_powers.index(q_power))+"]")
			x = xi[q_powers.index(q_power)] 
			
		else:
			x = (x + xi[q_powers.index(q_power)]*q_power) % (qi**ei) 
			print("Adding xi["+str(q_powers.index(q_power))+"]*"+str(q_power)+" to x")
			print("x is now: "+str(x))
	#raw_input('Waiting for user..')
	print("final x is now: "+str(x)+"mod "+str(qi)+"**"+str(ei))
	return x, qi**ei
