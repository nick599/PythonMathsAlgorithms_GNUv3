	...
	if result is False:
		...
		for C in C_list:
			...
			if Bi[i]==0 and zi[i]==1:
				...
				calc_xi_status = result[2]

				if calc_xi_status == False:
					#Calc_xi method in prop_234 failed. Now try brute force search
					...
					if status == True:
						...
						#exponent not found
						raw_input("Calc_xi method and brute force method failed to find x..")
						sys.exit()
				else:
		...
		elif calc_xi_status == False and brute_force_status == True:
			#no need to use CRT
		elif calc_xi_status == False and brute_force_status == False:
