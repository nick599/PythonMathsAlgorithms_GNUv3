...
def chinese_remainder(x, x_moduli):
	...
	cong_x=[]
	cong_moduli=[]
	for number in xrange(0,len(x)+1):
...
def dlp(g, h, p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln):
	...
	#now need to combine x's and x_moduli
	print('========================')
...
def prop_234(Bi, zi, qi, ei, p):
	...
	for q_power in q_powers:
		...
		if q_power==1:
			...
			if xi[0] > 0:
				...
				print("Calculating modular inverse of "+str(g)+"**"+str(-xi[0])+", mod "+str(p))				
				b = calc_modinverse(g, xi[0], p) #b = 5
				#print("b is: "+str(b)) #b = 5
				rhs = (h * b) % p #rhs = (19 * 5) % 101 = 95 % 101				
				#print("rhs is: "+str(rhs))
			elif xi[0]==0:
				#g**[x_0]=g**0=1, mod p
				#Hence h*(g**[-x_0]) = h, mod p
	...
	#for j in xrange(0, e+1): 
		#1st loop: x= x_0
		#2nd loop: x= x_0*q_power[1] + x_1*q_power[1]
		#3rd loop: x= x_0*q_power[0] + x_1*q_power[1] + x_2*q_power[2]
		if q_power==1: 
			...
			print "Adding "+str(xi[(q_powers.index(q_power))])+"*"+str(q_power)+" to x, and reducing mod "+str(p)
			x = (x + xi[q_powers.index(q_power)]*q_power) % p
		print("x is now: "+str(x))
	#raw_input('Waiting for user..')
	M = qi ** ei
	print("final x is now: "+str(x)+" mod "+str(M))		
	return x, M
