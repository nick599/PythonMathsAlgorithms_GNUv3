...
def main():
	...
	print('Looping through values - Running dlp() multiple times..')
		...
		for g in g_values:
			for p in p_values:
				...
				for h in h_values:			
					...
					result = dlp(g,h,p, p_values, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_prop234_soln, count_needlargerprimelist, answers_to_be_checked, count_notprime, count_ghp_checks_fail, count_notprimroot)
					...
					print "g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", x: "+str(result[0])+", count_notprimroot: "+str(count_notprimroot)+", no_solns: "+str(count_nosolns)+", need_largerprimelist: "+str(count_needlargerprimelist )				
...
def dlp(g, h, p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_prop234_soln, count_needlargerprimelist, answers_to_be_checked, count_notprime, count_ghp_checks_fail, count_notprimroot):
...
def calc_primes_powers_remainder(isliced_primes, N_remainder):
	...
	if not N_remainder:	
		...
		if N_remainder > long(isliced_primes[-1]) and math.sqrt(N_remainder) <= isliced_primes[-1]:
			...
			print "isliced_primes[-1] is: "+str(isliced_primes[-1])
		elif N_remainder > long(isliced_primes[-1]):
			print('There is a remainder of: '+str(N_remainder))
			print "math.sqrt(N_remainder) is: "+str(math.sqrt(N_remainder))		
			print "isliced_primes[-1] is: "+str(isliced_primes[-1])
		elif N_remainder <> 1:
