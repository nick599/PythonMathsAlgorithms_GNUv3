...
def main():
	...
	g_values = xrange(1,1001)
	h_values = xrange(1,1001)
	#p_values = primes

	total_values = len(primes)*len(list(g_values))*len(list(h_values))

	10_percent=total_values / 10
	20_percent=2*10_percent
	30_percent=3*10_percent
	40_percent=4*10_percent
	50_percent=5*10_percent
	60_percent=6*10_percent
	70_percent=7*10_percent
	80_percent=8*10_percent
	90_percent=9*10_percent
	100_percent=10*10_percent
	...
	for g in g_values:
		for h in h_values:
			for p in primes:
				...
				answers_to_be_checked.append(result[13])
...
def dlp(g, h, p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked):
