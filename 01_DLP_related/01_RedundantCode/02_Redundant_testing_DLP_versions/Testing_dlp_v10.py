...
def main():
	...
	print "len p_values was: "+str(len(p_values))
	
	result = new_primes(p_values)
	p_values = result[0]
	status = result[1]

	print "len p_values now: "+str(len(p_values))
		...
		for g in g_values:
			for p in p_values:
				...
				for h in h_values:			
					...
					print "g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", x: "+str(result[0])+", no_solns: "+str(count_nosolns)+", need_largerprimelist: "+str(count_needlargerprimelist )				
