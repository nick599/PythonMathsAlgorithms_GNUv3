...
def main():
	...
	Grand_total_values = len(p_values)*len(list(g_values))*len(list(h_values))
		...
		for p in p_values:
			...
			for h in h_values:
				for g in g_values:			
					...
					result = dlp(g,h,p, p_values, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_notprime, count_ghp_checks_fail)
					
					#return x_final, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, x_moduli_final, answers_to_be_checked, count_ghp_checks_fail, error, count_needlargerprimelist
					...
					count_normal_soln = count_normal_soln + result[6]
					...
					print "p: "+str(p)+", h: "+str(h)+", g: "+str(g)+", x: "+str(result[0])+", no_solns: "+str(count_nosolns)+", need_largerprimelist: "+str(count_needlargerprimelist )				
...
def g_prime_power_order(g_values, p_values):
	...
	for g in g_values:
		print "====================="
		print "g is: "+str(g)			
		#if prime_power_found == False:
			#raw_input("Waiting for user...")
		temp_ans_p=[]
		status_p_elements_same_values = False
		for p in p_values:
			if status_p_elements_same_values == True:
				break
			print "-------------------"
			print "p is: "+str(p)
			print "temp_ans_p: "+str(temp_ans_p)			
			#if len(temp_ans_p) > 3 and min(temp_ans_p) == max(temp_ans_p):
	...
	print "n values: 0 to "+str(max(len(n_values)))
	print "g values: "+str(min(len(new_g_values)))+" to "+str(max(len(new_g_values)))
	print "p values: "+str(min(len(new_p_values)))+" to "+str(max(len(new_p_values)))
...
def dlp(g, h, p, primes, count_nosolns, count_zi_bi_equal_1, count_bi_equal_1_zi_ntequal_0_1, count_x_equals_0, count_Bi_equals_zi, count_normal_soln, count_needlargerprimelist, answers_to_be_checked, count_notprime, count_ghp_checks_fail):
