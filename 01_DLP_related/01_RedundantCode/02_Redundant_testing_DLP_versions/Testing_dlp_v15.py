		...
		for g in g_values:
			for p in p_values:
				...
				if g == p:
					...
					#raw_input("Waiting for user..")				
					for h in h_values:			
						#print "type(h/g) is: "+str(type(h/g))
						#raw_input("Waiting for user..")						
						if type(h/g)== int:				
							print "h / g is an integer"
							#raw_input("Waiting for user..")
							#now g divides h. Now need to obtain value of n s.t. g**n = h
							result = exponent_g_n(g,h,p)
							#return x, status
							x=result[0]
							status=result[1]
							print "g: "+str(g)+", p: "+str(p)+", h: "+str(h)+". g divides h. x: is: "+str(x)+", notprimroot: "+str(count_notprimroot)+", no_solns: "+str(count_nosolns)+", largerprimelist: "+str(count_needlargerprimelist)
							#print "x is: "+str(x)
							raw_input("Waiting for user..")						
						else:
							...
							print "g: "+str(g)+", h: "+str(h)+", p: "+str(p)+", x: "+str(x_final)+", notprimroot: "+str(count_notprimroot)+", no_solns: "+str(count_nosolns)+", largerprimelist: "+str(count_needlargerprimelist)				
