...
def x_plus_a_cong_check(n, r, int_floor_sqrt_eulerphi_r_times_log_2_n):
	...
	#Formula for nCr coefficients! -> n! / r!*(n-r)! -> need formula for n!
	bin_coeffs=[]	
...
def Bin_Coeffs_withmod(n):
	...
	factors=factorise(b)
	fac_list=[]	
	status_onlycontains2 = True
	for factor in factors:
		if factor not in fac_list:
			#append to fac_list
			fac_list.append(factor)

	print "unique prime factors are: "+str(fac_list)

	#check if fac_list only contains 2 and no other elements
	for fac in fac_list:
		if fac <> 2:
			status_onlycontains2 == False
	...
	for b in xrange(0, n + 1):	
		if (n == b) or (b == 0):
			...
			coeffs.append(0)
			#return n
		elif status_onlycontains2 == True
			#print "coeff("+str(n)+", "+str(b)+") is 0"
			coeffs.append(0)
			#return 0
