...
def smallest_r(n , log_2_n_squared, log_2_n):
	...
	print "Running smallest_r(n , log_2_n_squared).."
	...
	#print "math.ceil(pow(math.log(n, 2), 5)) is: "+str(math.ceil(pow(math.log(n, 2), 5)))
	
	#order_n(a) = order of a modulo n
	#integer a, positive integer n with gcd(a,n)=1,
	#multiplicative order of a modulo n is the smallest integer k with pow(a,k,n)=1		
	...
	while r <= int(math.ceil(pow(log_2_n,5))):
		...
		order_result = calc_order(n) 
		#calc_order(g, p)
		#status is False for "no exponent found"	
		#status is True for exponent found
		order_r_n = order_result[0]					#O(1)
		status = order_result[1]					#O(1)
		#status = result[1]					
...
def exponent_g_n(g, h, p):
	print "Running exponent_g_n("+str(g)+", "+str(h)+", "+str(p)+").."
...
def calc_order(n):
	...
	print "Running calc_order("+str(n)+").."
	#print "Need to calculate & check order for: "+str(n)
	
	#use exponent function to get order of n mod r
	result = exponent_n_r(n, 1, r)				#Worst: O(n + 7)	#Best: O(8)
	#return x, status
	#status is False for "no exponent found"	
	#status is True for exponent found
	order = result[0]					#O(1)
