...
def main():
	...
	perfect_power_result = perfect_power_check_a_b(n)
	#return status
...
def perfect_power_check_a_b(n):
	print "Running perfect_power_check_a_b("+str(n)+").."
	...
	while a < n:					#O(n)	#Worst: O(n**2+5n) 	#Best: 6 operations
		...
		if status== True:
			...
			flr_log_na = math.floor(log_na)
			print "math.floor(math.log("+str(n)+","+str(a)+")) is: "+str(flr_log_na) 
			for b in xrange(1, math.floor(math.log(n,a))+1):	#O(n)	
